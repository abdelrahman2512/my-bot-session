...
async def start_app(c:Client,m:Message):
	...
	if not user == ME:
		...
		if do.count("left") or do.count("Bad Request: user not found"):
			name = (await app.get_chat("@xco_de")).first_name
			await m.reply(Sub.format(name,"https://t.me/xco_de"), 
			reply_markup=sub_bu,
...
async def add_dev(c:Client,m:Message):
	...
	if not await is_sudo(user_id=ask.text):
		...
		await add_sudo(user_id=ask.text)
		await m.reply(text.format(men))
	else:
		men = (await app.get_users(ask.text)).mention
...
async def del_dev(c:Client,m:Message):
	...
	if not is_sudo(user_id=ask.text):
		...
		men = (await app.get_users(ask.text)).mention
		await del_sudo(user_id=ask.text)
		await m.reply(f"**◍ تم تنزيل {men} من المطورين \n√**")
